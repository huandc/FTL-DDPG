...
class DataGenerator:
    ...
    def load_datas(self, datapath, itempath):
        ...
        data = pd.read_csv(datapath, sep="\t",
                           usecols=[0, 1, 2, 6],
                           dtype={"userId": np.int32, "itemId": np.int32, "rating": np.int8},
        ...
        item_titles = pd.read_csv(itempath, sep="\t", names=['itemName', 'itemId'],
                                  dtype={"itemId": np.int32, "itemName": np.int32},
                                  # dtype={"itemId": np.int32, "itemName": np.int32},
                                  usecols=[0, 1], encoding='latin-1', engine='python')
        print(len(item_titles))
        print(item_titles['itemId'].max())
        print("merge ing")
        data_merge = data.merge(item_titles, on='itemId', how='left')
...
if __name__ == '__main__':
    ...
    eg = EmbeddingsGenerator(dg.user_train,
                             pd.read_csv(datapath, sep="\t", encoding='latin-1', engine='python',
                                         usecols=[0, 1, 2, 6],
                                         dtype={"userId": np.int16, "itemId": np.int16, "rating": np.int8},
                                         names=['userId', 'itemId', 'rating', 'timestamp']))

    eg.train(nb_epochs=2500, batch_size=5000)
    # train_loss, train_accuracy = eg.test(dg.user_train)
    # print('Train set: Loss=%.4f ; Accuracy=%.1f%%' % (train_loss, train_accuracy * 100))
    # test_loss, test_accuracy = eg.test(dg.user_test)
    # print('Test set: Loss=%.4f ; Accuracy=%.1f%%' % (test_loss, test_accuracy * 100))
    eg.save_embeddings(path + "embeddings.csv")
    # eg.train(nb_epochs=1000, batch_size=5000)
    # eg.save_embeddings(path + "embeddings.csv")
