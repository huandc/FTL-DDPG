...
class Actor:
    ...
    def __init__(self, sess, state_space_size, action_space_size, batch_size, ra_length, history_length, embedding_size,
        ...
        with tf.variable_scope(self.scope):
            ...
            gradients = tf.gradients(
                tf.reshape(self.action_weights, [self.batch_size, self.action_space_size], name='60'),
                self.network_params,
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope(scope):
            ...
            x = tf.keras.layers.Dense(self.ra_length * self.embedding_size)(last_output)  # 输出
    ...
    def get_recommendation_list(self, ra_length, noisy_state, embeddings, target=False):
        ...
        weights = method(noisy_state, [ra_length * 2] * batch_size)
        ...
        # '8: return a_t'
        recommend_list = np.array([[embeddings.get_embedding(np.argmax(scores[i][k]))
                                    for k in range(ra_length)]
                                   for i in range(batch_size)])

        return recommend_list

    def set_actor_vars(self, global_vars):
    ...
    def get_actor_vars(self):
        actor_vars = self.sess.run(tf.trainable_variables())
        return actor_vars
