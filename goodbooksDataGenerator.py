...
class DataGenerator:
    ...
    def load_datas(self, datapath, itempath):
        ...
        data = pd.read_csv(datapath, sep=",",
                           # usecols=[0,9],
                           names=['itemId', 'userId', 'rating', 'timestamp'], encoding='latin-1',
                           engine='python')
        print("len data")
        print(len(data))
        ...
        movie_titles = pd.read_csv(itempath, sep=",", names=['itemId', 'itemName'],
                                   ...
                                   usecols=[0, 9], encoding='latin-1', engine='python')
        data_merge = data.merge(movie_titles, on='itemId', how='left')
        # return data_merge[:int(0.7 * len(data_merge))]
        return data_merge
...
if __name__ == '__main__':
    ...
    eg.train(nb_epochs=1500, batch_size=5000)
